...
Hit Box::intersect(Ray const& ray) {
    glm::vec3 inv_direction {1.0/ray.direction.x,1.0/ray.direction.y,1.0/ray.direction.z};

    float t1 = (maximum_.x - ray.origin.x) * inv_direction.x;
    float t2 = (minimum_.x - ray.origin.x) * inv_direction.x;
    float t3 = (maximum_.y - ray.origin.y) * inv_direction.y;
    float t4 = (minimum_.y - ray.origin.y) * inv_direction.y;
    float t5 = (maximum_.z - ray.origin.z) * inv_direction.z;
    float t6 = (minimum_.z - ray.origin.z) * inv_direction.z;
...
Hit Box::intersect(Ray const& ray) {
    ...
    Hit hits[6];
    hits[0] = planeHit(ray, vertices[0], vertices[1], vertices[2], vertices[3]); //ZY-plane, minimum
    hits[1] = planeHit(ray, vertices[4], vertices[5], vertices[6], vertices[7]); //ZY-plane, maximum
    ...
    hits[4] = planeHit(ray, vertices[0], vertices[2], vertices[4], vertices[6]); //XY-plane, minimum
    hits[5] = planeHit(ray, vertices[1], vertices[3], vertices[5], vertices[7]); //XY-plane, maximum
